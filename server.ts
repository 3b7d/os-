...
async function startServer() {
  ...
  app.get("/api/orders", async (req, res) => {
    try {
      ...
      // fallback to mock data since we don't have a real DB instance in the container
      // But according to PRD, we should try the query.
      const [rows] = await pool.query("SELECT * FROM delivery_orders");
      res.json(rows);
    } catch (error) {
      console.warn("MySQL query failed, falling back to mock data for demo.");
